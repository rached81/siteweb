...
// import { Alignment,AlignmentEditing, AlignmentUI } from '@ckeditor/ckeditor5-alignment/src';
...
const currentLocale = window.appLocale || 'fr'; // fallback si non défini
...
window.initEditor =  function(elementId, callback) {
    ClassicEditor
        ...
        .then( editor => {
            callback(editor);
            console.log(Array.from( editor.ui.componentFactory.names() ), currentLocale);
        } )
        .catch( error => {
            console.error( error );
        } )
